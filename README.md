...
Renpy-Compiler is a project that implements a complete **AST(1) construction and semantic analysis** pipeline for a Ren’Py-inspired scripting language. The compiler parses `.rpy` scripts and generates an abstract syntax tree (AST) that can be used by Python to execute and render a visual novel.
